...
from zsdg.enc2dec.decoders import DecoderPointerGen
...
class BaseModel(nn.Module):
    ...
    def ptr_loss(self, dec_ctx, labels):
        ...
        attention_loss = -1.0 * torch.log(mix_ptr.clamp(min=1e-8))
        attention_loss.masked_fill_(label_mask, 0)

        valid_cnt = (label_mask.size(0) - torch.sum(label_mask).float()) + 1e-8
