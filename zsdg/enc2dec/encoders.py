...
class Semantic_guide(nn.Module):
    def __init__(self, utt_cell_size, dropout,
                 ...
                 feat_size=0):
        super(Semantic_guide, self).__init__()
    ...
    def forward(self, utterances):
        ...
        result = torch.sum(words_embeded, 1)
        return result
...
class RnnUttEncoder(nn.Module):
    ...
    def forward(self, utterances, feats=None, semic=False, init_state=None, return_all=False):
        batch_size = int(utterances.size()[0])
        max_ctx_lens = int(utterances.size()[1])
        max_utt_len = int(utterances.size()[2])
        ...
        if semic == False:
            ...
            words_embeded=utterances
            if feats is not None:
                flat_feats = feats.view(-1, 1)
                flat_feats = flat_feats.unsqueeze(1).repeat(1, max_ctx_lens, 1)
                words_embeded = torch.cat([words_embeded, flat_feats], dim=2)
        ...
        if semic == False:
            ...
            utt_embedded = utt_embedded.view(batch_size, 1, self.output_size)
        if return_all:
