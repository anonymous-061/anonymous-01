...
class fn_softmax(nn.Module):
    def __init__(self, input_size, output_size, is_lstm, has_bias=True):
        super(fn_softmax, self).__init__()
        # if is_lstm:
        #     self.linear_h = nn.Linear(input_size, output_size, bias=has_bias)
        #     self.linear_c = nn.Linear(input_size, output_size, bias=has_bias)
        # else:
...
class LinearConnector_f(nn.Module):
    def __init__(self, input_size, output_size, is_lstm, has_bias=True):
        super(LinearConnector_f, self).__init__()
        if is_lstm:
            self.linear_h = nn.Linear(input_size, output_size, bias=has_bias)
            self.linear_c = nn.Linear(input_size, output_size, bias=has_bias)
        else:
...
class LinearConnector_vg(nn.Module):
    def __init__(self, input_size, output_size, is_lstm, has_bias=True):
        super(LinearConnector_vg, self).__init__()
        # if is_lstm:
        #     self.linear_h = nn.Linear(input_size, output_size, bias=has_bias)
        #     self.linear_c = nn.Linear(input_size, output_size, bias=has_bias)
        # else:
        self.linear_enc = nn.Linear(input_size, 200, bias=has_bias)
        self.linear_dec = nn.Linear(200, 512, bias=has_bias)
        self.is_lstm = is_lstm
        self.Relu = nn.ReLU()
        self.tanh = nn.Tanh()
    ...
    def forward(self, inputs):
        ...
        #     return (h, c)
        # else:
        h_enc_ = self.linear_enc(inputs)
        h_enc = torch.sum(h_enc_, 1)
        # out = self.linear_dec(self.tanh(h_enc_))
        # return self.Relu(out),self.tanh(h_enc)
        out = self.linear_dec(self.Sigmoid(h_enc_))
        return self.Relu(out), self.Sigmoid(h_enc)
    ...
    def get_w(self):
        if self.is_lstm:
            ...
            return self.linear.weight
# class calculate_T(nn.Module):
#     def __init__(self, input_size, output_size, is_lstm, has_bias=True):
#         super(calculate_T, self).__init__()
#         # if is_lstm:
#         #     self.linear_h = nn.Linear(input_size, output_size, bias=has_bias)
#         #     self.linear_c = nn.Linear(input_size, output_size, bias=has_bias)
#         # else:
