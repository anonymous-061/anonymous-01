...
class SimDialDataLoader(LongDataLoader):
    ...
    def epoch_init(self, config, shuffle=True, verbose=True):
        ...
        if shuffle:
            temp_batch_id = range(len(self.warmup_flags))
            np.random.shuffle(list(temp_batch_id))
    ...
    def next_batch(self):
        # if self.ptr < self.num_batch:
        if self.ptr < len(self.grid_indexes):

            is_warmup = self.warmup_flags[self.ptr]
            current_grid = self.grid_indexes[self.ptr]
            # for ijjj in range(len(self.warmup_flags)):
            #     if self.warmup_flags[ijjj] == True:
            #         print('%s step is_warmup************************ %s'%ijjj)
            # is_warmup = True
            if is_warmup:
                self.ptr += 1
                print("********************************************")
    ...
    def _prepare_batch(self, cur_grid, prev_grid):
        ...
        vec_out_lens_warm, vec_out_utts_warm, vec_out_acts_warm, domains_warm, vec_domain_metas_warm,vec_out_acts_semtic = \
            self._prepare_warmup_inside(cur_grid)
        vec_acts_warm_S = vec_out_utts_warm[0:100],vec_out_acts_warm[0:100],vec_out_acts_semtic[0:100],vec_out_lens_warm[0:100]
        vec_acts_warm_T = vec_out_utts_warm[100:],vec_out_acts_warm[100:],vec_out_acts_semtic[100:],vec_out_lens_warm[100:]

        # vec_acts_warm_S = vec_out_acts_semtic[0:100]
        # vec_acts_warm_T = vec_out_acts_semtic[100:]
        ...
        out_utts, out_lens = [], []
        out_acts, out_act_lens = [], []
        out_acts_semtic,out_acts_semtic_len=[],[]
        ctx_acts_semtic,ctx_acts_semtic_len=[],[]
        ...
        for row in rows:
            if s_id < len(row) - 1:
                ...
                # print(out_row.speaker)
                out_utt = out_row.utt

                # source context
                cxt_lens.append(len(in_row))
                batch_ctx, batch_confs = [], []
                batch_acts_semtic, batch_acts_semtic_len = [], []
                for turn in in_row:
                    batch_ctx.append(self.pad_to(self.max_utt_size, turn.utt))
                    # print(self.max_utt_size)
                    batch_confs.append(turn.conf)
                    if "actions_semtic" not in turn:
                        batch_acts_semtic.append([0,0,0,0,0])
                    else:
                        batch_acts_semtic.append(self.pad_to(5, turn.actions_semtic))

                ctx_utts.append(batch_ctx)
                ctx_confs.append(batch_confs)
                ctx_acts_semtic.append(batch_acts_semtic)
                ...
                out_acts_semtic.append(out_row.actions_semtic)
                out_acts_semtic_len.append(len(out_row.actions_semtic))
        ...
        vec_ctx_acts_semtic = np.zeros((self.batch_size, 5), dtype=np.float32)
        ...
        vec_out_acts_semtic =np.zeros((self.batch_size, np.max(out_acts_semtic_len)), dtype=np.int32)
        # vec_out_acts_semtic =np.zeros((self.batch_size, 5), dtype=np.int32)
        ...
        for b_id in range(self.batch_size):
            vec_out_utts[b_id, 0:vec_out_lens[b_id]] = out_utts[b_id]
            vec_out_acts[b_id, 0:out_act_lens[b_id]] = out_acts[b_id]
            vec_out_acts_semtic[b_id, 0:out_acts_semtic_len[b_id]] = out_acts_semtic[b_id]
            ...
            ###############################################################################################################
            # vec_ctx_acts_semtic[b_id, 0:vec_ctx_lens[b_id], :] = vec_ctx_acts_semtic[b_id]
            ###########################################################################################################
        ...
        return Pack(context_lens=vec_ctx_lens, contexts=vec_ctx_utts, context_confs=vec_ctx_confs,
                    ...
                    domains=domains, domain_metas=vec_domain_metas,vec_acts_warm_S =vec_acts_warm_S,
                    vec_acts_warm_T=vec_acts_warm_T,output_acts_semtic=vec_out_acts_semtic,ifwarm=False)

    def _prepare_warmup_inside(self, selected_ids):
        # the batch index, the starting point and end point for segment
        rows = self.warmup_data
        out_utts, out_lens = [], []
        out_acts, out_act_lens = [], []
        domains, domain_metas = [], []
        out_acts_semtic, out_acts_semtic_len = [], []
        ...
        for row in rows:
            ...
            out_acts_semtic.append(row.actions_semtic)
            out_acts_semtic_len.append(len(row.actions_semtic))
        ...
        vec_out_utts = np.zeros((lenwarm, np.max(out_lens)), dtype=np.int32)
        vec_out_acts = np.zeros((lenwarm, np.max(out_act_lens)), dtype=np.int32)
        vec_out_acts_semtic = np.zeros((lenwarm, np.max(out_acts_semtic_len)), dtype=np.int32)
        vec_domain_metas = np.zeros((lenwarm, self.max_utt_size), dtype=np.int32)

        for b_id in range(lenwarm):
            vec_out_utts[b_id, 0:vec_out_lens[b_id]] = out_utts[b_id]
            vec_out_acts[b_id, 0:out_act_lens[b_id]] = out_acts[b_id]
            vec_out_acts_semtic[b_id, 0:out_acts_semtic_len[b_id]] = out_acts_semtic[b_id]
            vec_domain_metas[b_id, :] = domain_metas[b_id]
        ...
        return vec_out_lens, vec_out_utts, vec_out_acts,domains,vec_domain_metas,vec_out_acts_semtic


    def _prepare_warmup_batch(self, selected_ids):
        # the batch index, the starting point and end point for segment
        vec_out_lens_warm, vec_out_utts_warm, vec_out_acts_warm, domains_warm, vec_domain_metas_warm, vec_out_acts_semtic = \
            self._prepare_warmup_inside(0)
        vec_acts_warm_S = vec_out_utts_warm[0:20], vec_out_acts_warm[0:20], vec_out_acts_semtic[
                                                                            0:20], vec_out_lens_warm[0:20]
        vec_acts_warm_T = vec_out_utts_warm[100:], vec_out_acts_warm[100:], vec_out_acts_semtic[
                                                                            100:], vec_out_lens_warm[100:]
        ...
        for row in rows:
            ...
            out_acts_semtic.append(row.actions)
            out_acts_semtic_len.append(len(row.actions))
        ...
        return Pack(output_lens=vec_out_lens, outputs=vec_out_utts, output_actions=vec_out_acts,
                    domains=domains, domain_metas=vec_domain_metas,vec_acts_warm_S =vec_acts_warm_S,
                    vec_acts_warm_T=vec_acts_warm_T,output_acts_semtic=vec_out_acts_semtic,ifwarm = True)
