...
class SimDialCorpus(object):
    ...
    def _process_meta(self, domain_meta):
        ...
        for domain_name, domain in domain_meta.items():
            ...
            for slot, examples in nlg_spec.items():
                ...
                # norm_meta["allslots"] = vocab_list
            all_norm_meta[domain_name] = norm_meta
            all_norm_meta["rest_slots"] = norm_meta
        # END OF reading
    ...
    def _act_to_str(self, speaker, act):
        ...
        return [intent] + list(str_paras)

    def _act_to_str_only(self, speaker, act):
        # paras = act['parameters']
        intent = act['act']
        # str_paras = []
        # if speaker == SYS and intent == 'inform':
    ...
    def _to_id_corpus(self, name, data, use_black_list):
            ...
            for msg in dialog:
                ...
                if 'actions' in copy_msg.keys():
                    ...
                    for act in msg.actions:
                        ...
                        tokens_semtic = self._act_to_str_only(msg.speaker, act)
                        str_acts.append(' '.join(tokens))
                        str_acts_semtic.append(' '.join(tokens_semtic))
                    str_acts = self.tokenize("|".join(str_acts))
                    str_acts_semtic = self.tokenize(" ".join(str_acts_semtic))

                    if self.include_domain and False:
                        str_acts = [msg.speaker, domain] + str_acts
                    else:
                        str_acts = [msg.speaker] + str_acts
                        str_acts_semtic = [msg.speaker] +[domain]+ str_acts_semtic
                    tkn_acts = [self.rev_vocab[t] for t in str_acts]
                    tkn_acts_semtic = [self.rev_vocab[t] for t in str_acts_semtic]
                    tkn_acts_semtic = list(set(tkn_acts_semtic))
                    copy_msg['actions'] = tkn_acts
                    copy_msg['actions_semtic'] = tkn_acts_semtic
                    howmanyactionsemtic.append(str(tkn_acts_semtic))
        ...
        # print(len(set(howmanyactionsemtic)))
        return results,set(howmanyactionsemtic)
    ...
    def get_dialog_corpus(self):
        ...
        id_train,train_sem = self._to_id_corpus('train', train_corpus, use_black_list=True)
        id_valid,valid_sem = self._to_id_corpus('valid', valid_corpus, use_black_list=True)
        id_test,test_sem = self._to_id_corpus('test', self.test_corpus, use_black_list=False)
    ...
    def get_seed_responses(self, utt_cnt=100, domains=None, speakers=None):
        ...
        for domain, ids in id2domains.items():
            ...
            if domains is None or domain in domains:
                selected_ids.extend(ids[0:int(dialog_cnt)])

            data = [self.corpus[idx] for idx in selected_ids]
            id_selected_data,_ = self._to_id_corpus('Extra', data, use_black_list=False)
